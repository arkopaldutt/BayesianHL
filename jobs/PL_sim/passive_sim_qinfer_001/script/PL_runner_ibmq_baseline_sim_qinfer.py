...
from bayesian_learner import ExperimentRunner
...
moset = [0,1,2]
prepset = [0,1]
time_stamps = np.linspace(1.0e-7, 6e-7, 3*81)
...
if FLAG_simulator:
    SAVE_DIR = policy_type + '_baseline_sim_qinfer_%03d' % expt_number
else:
    SAVE_DIR = policy_type + '_baseline_expt_qinfer_%03d' % expt_number
...
PL_baseline_expt_run = ExperimentRunner(J_truth_nd, query_space, xi_J, xi_t, FLAG_control_noise=FLAG_control_noise,
...
if __name__ == '__main__':
    ...
    results_PL = PL_baseline_expt_run.PL_runner(log_file=log_file_run)
