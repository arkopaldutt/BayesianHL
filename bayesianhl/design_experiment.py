...
class ActiveLearner(object):
    ...
    def __init__(self, policy_type='FIR', FLAG_normalization=False,
        ...
        TODO:
        Make the constraints more general. Currently, they also only work for the fixed query space.
        Needs to be updated for the adaptive query space case.
        """
    ...
    def optimal_query_distribution(self, A_cr, qs_model, p_ref=None, threshold_query=1e-5,
        ...
        valid_action_set = np.arange(N_config)
        action_list = A_cr.action_space

        # If filtering possible, do so!
        if hasattr(A_cr, 'nonempty_actions') and hasattr(A_cr, 'filtered_actions'):
            # Needs to be updated when we give more options such as uncertainty filtering
            # right now only non-empty
            # Holds indices of actions
            valid_action_set = A_cr.nonempty_actions()
        ...
        if self.policy_type == 'FIR':
            ...
            # Sourati (2018) algorithm is the same as
            # Chaudhuri (2015) algorithm by setting the testing FI to be identity
        ...
        if self.FLAG_constraints:
            ...
            if self.query_constraints['FLAG_lower_limits']:
                # TODO: Needs to be updated according to the valid action set business
...
def optimal_scalings(qs_model, query_space, xi_t, n_batches, max_k, HAL_FI_param,
    ...
    A_cr = action_space.Action_Space(moset, prepset, time_stamps_nd, xi_t,
    ...
    for k in range(1, max_k):
        if growth_time_stamps is not None:
            ...
            A_cr = action_space.Action_Space(moset, prepset, time_stamps, xi_t,
...
def policy_scalings(i_run, qs_model, moset, prepset, time_stamps, xi_t, n_batches, max_k, env_qs=None,
    ...
    # Initialize action space
    #A_cr = action_space.Action_Space(moset, prepset, time_stamps, xi_t, xi_param)
    A_cr = action_space.Action_Space(moset, prepset, time_stamps, xi_t,
                                        xi_J=qs_model.xi_J, xi_param=qs_model.xi_param, freq_convert=freq_convert)
