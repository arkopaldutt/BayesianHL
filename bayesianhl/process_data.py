...
def plot_classified_data(classifier, train_data, train_labels, xlim=None, ylim=None,
                         savefilename_fig='binary_classification',
    ...
    if FLAG_save is True:
        print('saving file')
        save_filename = savefilename_fig + '.png'
    ...
    # heatmap, xedges, yedges = np.histogram2d(train_data[:,0],train_data[:,1],bins=100)
    # extent = [xedges[0], xedges[-1], yedges[0], yedges[-1]]
    #
    # plt.clf()
    # plt.imshow(heatmap.T, extent=extent, origin='lower', cmap='bwr')
    # plt.xlim(xlim)
    # plt.ylim(ylim)
    # plt.xlabel('real(c)')
    # plt.ylabel('imag(c)')
...
def plot_calibration_gmm(gmm0, gmm1, train_data, train_labels, savefilename_fig='gmm_classification', FLAG_save=False):
    xlim = [np.min(train_data[:, 0]), np.max(train_data[:, 0])]
    ylim = [np.min(train_data[:, 1]), np.max(train_data[:, 1])]
...
def create_queryable_dataset_ibmq(expt_data, FLAG_classification=False, FLAG_merging=False):
    ...
    TODO: Generalize computation of N_actions and N_actions_t; for the case of adaptive query space or the like
    """
...
## A LOT OF VERY ROUGH CODE
def readout_calibration_classifier_query(ind_query, cal_data, cal_labels, samples_expt, nshots_cal,
                                         xlim=None, ylim=None, do_plot=True, FLAG_save=False):
    """
    Args:
        cal_data: Implicitly, am assuming that the first row corresponds to |0> and second row corresponds to |1>
        cal_labels:
    ...
    TODO: Generalize and make this later a part of single-qubit noise models from data
    """
    # Define training data and labels
    train_data_expt = np.reshape(cal_data, -1)
    train_data = np.array([train_data_expt.real, train_data_expt.imag]).T
    ...
    if do_plot or FLAG_save:
        title_fig_test = 'Classification of Query %d' % ind_query
        plot_classified_data(gnb, test_data, test_labels, xlim=xlim, ylim=ylim, title_fig=title_fig_test, show_plot=do_plot, FLAG_save=FLAG_save)

    return test_labels, misclassif_error_0_as_1, misclassif_error_1_as_0


def plot_ibmq_device_query(ind_query, data_file_name='ibmq_boel_fixed_qs_data_1.pickle', FLAG_classification=True,
                             do_plot=True, FLAG_save=False):
    """
    Function to create dataset from pickle files dumped from the new device code
    ...
    # IQ Data
    nshots = data['nshots']
    samples_expt = data['offline_data'][ind_query,:]
    ...
    # hard coded 80 and 81; change later
    delta_time = (last_time_stamp - first_time_stamp) / 80
    sample_freq = 1.0 / delta_time
    freq_convert = sample_freq * 2.0 * np.pi / 81
    ...
    if FLAG_classification:
        test_labels, r0, r1 = readout_calibration_classifier_query(ind_query, cal_data, cal_labels, samples_expt, nshots,
                                                             xlim=[-82,-40], ylim=[-40,40], do_plot=do_plot, FLAG_save=FLAG_save)

        ds = {'mvec': mvec_expt, 'uvec': uvec_expt, 'tvec': tvec_expt, 'time_stamps': time_stamps_expt,
              'samples': test_labels, 'misclassif_error': [r0,r1],
              'n_samples_m_u':nshots, 'FLAG_classification':FLAG_classification, 'first_time_stamp':first_time_stamp,
                'last_time_stamp':last_time_stamp, 'delta_time':delta_time, 'sample_freq':sample_freq,
                'freq_convert':freq_convert, 'device': data['device']}
    else:
        samples_p0, samples_p1 = readout_calibration_GMM(cal_data, samples_expt, nshots,
                                                         do_plot=do_plot, FLAG_save=FLAG_save)

        ds = {'mvec': mvec_expt, 'uvec': uvec_expt, 'tvec': tvec_expt, 'samples': samples_expt,
              'time_stamps': time_stamps_expt, 'samples_p0': samples_p0, 'samples_p1': samples_p1,
              'n_samples_m_u':nshots, 'FLAG_classification': FLAG_classification, 'first_time_stamp':first_time_stamp,
                'last_time_stamp':last_time_stamp, 'delta_time':delta_time, 'sample_freq':sample_freq,
                'freq_convert':freq_convert, 'device': data['device']}
    return ds
